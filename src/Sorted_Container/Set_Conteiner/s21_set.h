...
class set : public Common_s_m<T> {
 ...
 public:
  ...
  set(std::initializer_list<value_type> const &items) {
    tree = new Red_black_tree<T>();  // GCOVR_EXCL_LINE
  ...
  //     if(!tree) throw std::out_of_range("Дерево пустое");
  //     bool flag = tree -> remove(pos.get_data());
  //     nodes_counter--;
  //     //else throw std::out_of_range("Удалять нечего");
  // };

  virtual void erase(iterator pos) override {
    if (empty()) {
      throw std::out_of_range("Дерево пустое");  // GCOVR_EXCL_LINE
    }
    if (!pos.get_curr())
      throw std::logic_error(
          "You try to erase empty position");  // GCOVR_EXCL_LINE
