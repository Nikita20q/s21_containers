...
class queue : public Common_q_s<T> {
 ...
 public:
  ...
  void push(const_reference &value) {
    if (tail == head && !head) {
      tail = new Node(value);  // GCOVR_EXCL_LINE
      head = tail;
    } else {
      tail->next = new Node(value);  // GCOVR_EXCL_LINE
      tail = tail->next;
    }
  ...
  const_reference front() {
    if (!head) throw std::out_of_range("Queue is empty");  // GCOVR_EXCL_LINE
    return head->data;
  };

  const_reference back() {
    if (!tail) throw std::out_of_range("Queue is empty");  // GCOVR_EXCL_LINE
    return tail->data;
  };
